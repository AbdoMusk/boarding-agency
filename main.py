...
# Models
OCR_MODEL = "glm-ocr"           # Ollama OCR model
LLM_MODEL = "gemma3"          # Ollama LLM model
# ==================
...
def encode_image(path):
    """
    Read and base64 encode an image so we can send it to Ollama.
    """
...
def run_ocr_on_images(image_files):
    """
    Sends images one by one to GLM-OCR
    """
    DebugFormatter.print_step(2, "Running OCR on Images")
    full_text = ""
    for idx, img in enumerate(image_files):
        ...
        response = ollama.chat(
            ...
            messages=[{
                "role": "user",
                "content": "Extract all text from this image. don't forget to extract text on the side also, so any character on the image extract it.",
                "images": [b64]
            }]
        )
...
def ask_llm_to_structure(cv_text):
    """
    Send the full extracted text to the LLM for JSON structuring
    """
    DebugFormatter.print_step(3, "Structuring CV with LLM")
...
def process_all_cvs():
    DebugFormatter.print_header("CV Processing Pipeline Started")
